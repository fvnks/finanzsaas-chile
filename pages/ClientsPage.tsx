...
interface ClientsPageProps {
  ...
  onAddSupplier?: (supplier: Omit<Supplier, 'id' | 'companyId'>) => Promise<void>;
...
const ClientsPage: React.FC<ClientsPageProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'clients' | 'suppliers'>('clients'); // Tab State
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
            <Building2 className="text-blue-600" /> Directorio Comercial
          </h2>
          <p className="text-slate-500">Gestión centralizada de Clientes y Proveedores.</p>
        </div>
        {/* Action buttons could go here if needed per tab */}
      </div>
      ...
      <div className="flex p-0.5 bg-slate-100 rounded-lg w-fit">
        <button
          ...
          className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'clients' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
        >
        ...
        <button
          ...
          className={`px-4 py-1.5 rounded-md text-xs font-bold transition-all flex items-center gap-2 ${activeTab === 'suppliers' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
        >
      ...
      {activeTab === 'suppliers' ? (
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div className="text-lg font-bold text-slate-700">Listado de Clientes</div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-slate-50/50 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  <tr>
                    <th className="px-4 py-2">Razón Social / RUT</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {paginatedClients.length === 0 ? (
                    <tr><td colSpan={4} className="p-4 text-center text-slate-500 text-xs">No hay clientes registrados.</td></tr>
                  ) : (
                    paginatedClients.map((client) => (
                      <tr key={client.id} className="hover:bg-slate-50/50 transition-colors group">
                        <td className="px-4 py-2">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center font-black text-xs shadow-inner shrink-0">
                              {(client.razonSocial || '?').charAt(0)}
                            </div>
                            <div>
                              <div className="font-bold text-slate-800 text-xs">{client.razonSocial}</div>
                              <div className="text-[10px] text-slate-400 font-bold">{client.rut}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-4 py-2">
                          <div className="flex flex-col gap-0.5">
                            {client.email && (
                              <div className="flex items-center text-[10px] text-slate-600">
                                <Mail size={10} className="mr-1 text-slate-400" /> {client.email}
                              </div>
                            )}
                            {client.telefono && (
                              <div className="flex items-center text-[10px] text-slate-600">
                                <Phone size={10} className="mr-1 text-slate-400" /> {client.telefono}
                              </div>
                            )}
                          </div>
                        </td>
                        <td className="px-4 py-2">
                          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] bg-green-50 text-green-700 font-bold uppercase tracking-wider">
                            <UserCheck size={9} className="mr-1" /> Verificado
                          </span>
                        </td>
