...
import { getInvoiceTypeLabel, normalizeInvoiceType, mapTypeToEnum } from '../src/utils/invoiceUtils';
...
const InvoicesPage: React.FC<InvoicesPageProps> = ({ invoices, expenses, clients, suppliers = [], costCenters, projects, onAdd, onUpdate, onDelete, onAddSupplier, currentUser }) => {
  ...
  return (
    <div className="space-y-6">
                ...
                <React.Fragment key={inv.id}>
                  <tr className={`hover:bg-slate-50/50 transition-colors group ${inv.status === 'CANCELLED' ? 'opacity-60 bg-red-50/30' : ''}`}>
                    ...
                    <td className="px-6 py-4 text-center">
                      {/* Days Overdue Calculation and Payment Date Display */}
                      {inv.paymentStatus === 'PAID' && inv.paymentDate ? (
                        <div className="inline-flex items-center justify-center px-2 py-1 rounded-lg border text-green-700 bg-green-50 border-green-200" title="Fecha de Pago">
                          <Calendar size={12} className="mr-1" />
                          <span className="text-[10px] font-bold">
                            {new Date(inv.paymentDate).toLocaleDateString('es-CL', { timeZone: 'UTC' })}
                          </span>
                        </div>
      ...
      {/* Payment Date Modal */}
      {showPaymentDateModal && invoiceToPay && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-[60]">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden border border-slate-200 animate-in fade-in zoom-in duration-200">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setShowPaymentDateModal(false)} className="text-slate-400 hover:text-slate-600">
                  <X size={20} />
                </button>
              </div>
              <p className="text-sm text-slate-500 mb-4">
                Indique la fecha en que se realizó el pago de la factura <span className="font-bold text-slate-800">{invoiceToPay.number}</span>.
              </p>
              <div className="space-y-2 mb-6">
                <label className="text-xs font-bold text-slate-600 uppercase">Fecha de Pago</label>
                <input
                  type="date"
                  className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none font-bold text-slate-700"
                  value={paymentDate}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-600 font-bold rounded-xl transition-all"
                >
                  Cancelar
                </button>
                <button
                  onClick={() => {
                    onUpdate({ ...invoiceToPay, paymentStatus: 'PAID', isPaid: true, paymentDate });
