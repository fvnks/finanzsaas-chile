    ...
    try {
        ...
        if (ufValue) {
            await prisma.exchangeRate.upsert({
                where: { date_currency: { date: todayStr, currency: 'UF' } },
                update: { value: ufValue },
                create: { date: todayStr, currency: 'UF', value: ufValue }
            });
        ...
        if (usdValue) {
            await prisma.exchangeRate.upsert({
                where: { date_currency: { date: todayStr, currency: 'USD' } },
                update: { value: usdValue },
                create: { date: todayStr, currency: 'USD', value: usdValue }
            });
